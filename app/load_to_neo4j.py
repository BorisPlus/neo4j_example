...
if __name__ == '__main__':
    ...
    # Connect with Neo4j
    _driver = GraphDatabase.driver('bolt://127.0.0.1:7687', auth=('neo4j', 'neo4j'))
    ...
    for dict_row in get_dict_row(file_path):
        ...
        id_node, id_node_created = Id(value=dict_row.get('ID')).save(_driver)

        # Node named Metro
        metro, metro_node_created = Metro(name=dict_row.get('Metro')).save(_driver)
        related_nodes.append((metro, metro_node_created))

        # Node named Weight
        weight, weight_node_created = Weight(value=dict_row.get('Weight')).save(_driver)
        related_nodes.append((weight, weight_node_created))

        # Node named Height
        height, height_node_created = Height(value=dict_row.get('Height')).save(_driver)
        related_nodes.append((height, height_node_created))

        # Node named Boobs
        boobs, boobs_node_created = Boobs(value=dict_row.get('Boobs')).save(_driver)
        related_nodes.append((boobs, boobs_node_created))

        # Node named Size
        size, size_node_created = Size(value=dict_row.get('Size')).save(_driver)
        related_nodes.append((size, size_node_created))

        # Node named Price
        price, price_node_created = Price(value=dict_row.get('Price')).save(_driver)
